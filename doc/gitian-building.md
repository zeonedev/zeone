...
*Setup instructions for a Gitian build of Zeone Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Zeone
...
- [Building Zeone Core](#building-zeone-core)
...
Clone the git repositories for Zeone Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/zeonecoin/zeone
```
...
in the Zeone Core repository under 'Fetch and build inputs' to install sources which require
...
To build Zeone Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Zeone Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zeone/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zeonecoin/zeone
...
For example:
```bash
URL=https://github.com/crowning-/zeone.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zeone=${COMMIT} --url zeone=${URL} ../zeone/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zeone=${COMMIT} --url zeone=${URL} ../zeone/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zeone=${COMMIT} --url zeone=${URL} ../zeone/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zeone git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zeone/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zeone.git
SIGPATH=/some/root/path/zeone-detached-sigs.git

./bin/gbuild --url zeone=${BTCPATH},signature=${SIGPATH} ../zeone/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zeone-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zeone-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zeone-osx-build.assert
```
...
[zeone/gitian.sigs](https://github.com/zeonecoin/gitian.sigs/) repository, or if that's not possible to create a pull
request.
