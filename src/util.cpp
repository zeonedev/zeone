...
const char * const BITCOIN_CONF_FILENAME = "zeone.conf";
const char * const BITCOIN_PID_FILENAME = "zeoned.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "zeone" is a composite category enabling all Zeone-related debug output
            if(ptrCategory->count(string("zeone"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZeoneCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZeoneCore
    // Mac: ~/Library/Application Support/ZeoneCore
    // Unix: ~/.zeonecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZeoneCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZeoneCore";
#else
    // Unix
    return pathRet / ".zeonecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty zeone.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override zeone.conf
