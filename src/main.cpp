...
#if defined(NDEBUG)
# error "Zeone Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    if (nPrevHeight == 0) {
        return 80000 * COIN;
	} else if (nPrevHeight <= 1500) {
        return 0.1 * COIN;
	} else if (nPrevHeight <= 4500) {
        return 0.5 * COIN;
	} else if (nPrevHeight <= 9000) {
        return 2.5 * COIN;
	} else if (nPrevHeight <= 15500) {
        return 6 * COIN;
	} else if (nPrevHeight <= 32000) {
        return 15 * COIN;
	} else if (nPrevHeight <= 48000) {
        return 30 * COIN;
	} else if (nPrevHeight <= 98000) {
        return 40 * COIN;
	} else if (nPrevHeight <= 112000) {
        return 10 * COIN;
...
{
        if (nHeight <= 3000) {
	return blockValue/2*1; // 50%
    } else if (nHeight <= 112000) {
	return blockValue/20*19; // 90%
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // ZEONE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ENZO): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ENZO): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ZEONE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ENZO): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ENZO): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /* 
        Zeone Related Inventory Messages
